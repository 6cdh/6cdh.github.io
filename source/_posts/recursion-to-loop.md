...
注意 `factorial_v1` 虽然末尾进行了递归, 但不是尾递归, 因为它最后执行的操作是加法而不是递归调用.
...
所以 `factorial_v1` 所需时间是 $O(n)$, 递归保存状态所需空间是 $O(n)$. 理想的迭代应该使用 $O(1)$ 的空间.
...
现在的 `factorial_impl` 接收两个参数, 将最后本该需要的加法转换到了参数上, 且是尾递归. 这很容易被编译器优化成等价的尾部 goto:
