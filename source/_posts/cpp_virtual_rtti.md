...
这个例子创建了一个 Derived 类的对象 derived_obj, 并使用一个指向 Base 类的引用 base_ref 来指向它. Base 类和 Child 类都重载了虚函数 vf(), 最后对 base_ref 调用 vf().
...
那么 `sizeof(Base)` 会返回 4, 即它的成员 `m_base` 的长度 (可自行验证). 而增加了虚函数 vf() 的 Base 类会在成员变量前面加上一个指向 `vtable_for_{classname} + 16` 的指针, 即指向虚函数表的指针, 其中`{classname}` 是类的名字, 此时 Base 类的大小增加到了 12, 由于结构体对齐, Base 的大小会进一步增长到 16, `sizeof(Base)` 得到的结果也是 16.
...
虚函数和 RTTI 虽然看起来没什么关系, 但由于其运行时的特性, 都是使用 vtable 中实现的. 如果对 vtable 实现的细节感兴趣, 可以参考 [Itanium ABI](https://itanium-cxx-abi.github.io/cxx-abi/abi.html#vtable) 的相关约定.
